...
- Respecter les règles d'architecture du projet (voir `context/CONTEXT.md`) : `core/` reste pur Dart, services stateless sans `notifyListeners`, modèles testés via `copyWith`.
...
Ces fichiers portent des règles métier fixées au fil de plusieurs sprints (voir `context/CONTEXT.md`) et n'ont aucun filet de sécurité. Écrire des tests de régression sur le comportement actuel avant tout changement, plutôt que de faire confiance à une relecture manuelle.
