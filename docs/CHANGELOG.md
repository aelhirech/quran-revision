...
| P3 | **Hadith/streak réimplémentés dans `learn_surah_screen.dart` au lieu de `HadithCard`/`StreakCard`** | Non traité au sprint dette §8.5 (2026-09-13) : la mise en page y est réellement différente (badge streak compact dans l'AppBar vs `StreakCard` pleine largeur, hadith en simple texte centré vs `HadithCard` encartée) — un remplacement changerait visiblement l'écran sans pouvoir être vérifié visuellement sur cette machine (§12 build bloqué). Filtre de recherche sourate dupliqué (`OnboardingScreen`/`SouratePickerSheet`/`ProfileScreen`) et `HistoryCard` (14 sessions demandées, 7 affichées) traités le même sprint. |
