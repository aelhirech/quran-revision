...
|----------|---------|-------|
| ~~P1~~ | ~~Config modifiable pendant un jour en attente de check-out~~ | **Traité 2026-09-13** (sprint `feature/phase-13-sprint1-config-lock-pending`) : `AppState.saveConfig` et `AppState.setShuffleEnabled` refusent maintenant toute modification (retournent `false`, rien n'est persisté) tant qu'un jour antérieur reste non scellé (`pendingDate != null`) — `setShuffleEnabled` a été trouvé exposé au même risque en `/code-review` du sprint (buildCycle réordonne aussi sur `shuffleEnabled`, pas seulement sur `selections`) et corrigé dans la foulée. `ProfileScreen`/`SettingsCard` affichent un SnackBar (`S.configBloqueeJourEnAttente`) au lieu de silencieusement ignorer l'action. `setPagesPerDay` restait exempté à raison (son guard `todayClosed` couvre un risque différent, voir `app_state_dayplan.dart`). Régression verrouillée par 2 tests dans `test/state/app_state_checkin_test.dart`. |
