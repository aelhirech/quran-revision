...
- **`markUnitsReached(units)`** — appelé à la fin d'une manche PlanScreen (`reach=1` en batch, `AyahFactsService.setReachForUnits`). **N'avance plus `cyclePosition`** — voir `checkOut` ci-dessous. Reste utile pour la déclaration "une part faite"/"tout fait" du modal d'engagement (`_CommitmentSheet`) et la saisie manuelle (`ManualSessionSheet`), qui couvrent des unités sans passer par les cases à cocher individuelles de `toggleTodayUnitReach` ci-dessous.
